...
import aurita.daos.map.UserMapSettingsDAOInterface
...
trait UserXLoginInfoDAO extends CRUD[UserXLoginInfo] {
   ...
   */
  def insertIdentity(
    identity: IdentityImpl, mapSettings: MapSettings
  ): Future[(UserXLoginInfo, User)]
}
...
trait UserXLoginInfoDAOInterface extends DAOHelpers
  ...
  class UserXLoginInfoDAOImpl() extends UserXLoginInfoDAO with CRUDImpl[
    ...
    protected lazy val userMapSettingsTableQuery = TableQuery[UserMapSettingsTable]
    ...
    def insertIdentity(
      ...
      db.run(
        for {
          ...
          _ <- userMapSettingsTableQuery.filter(
            _.userId === identity.user.id
          ).result.headOption flatMap {
            case Some(ums0) => slick.dbio.DBIOAction.failed(
              new Exception(s"Duplicate map settings exists: ${ums0}")
            )
            case None => slick.dbio.DBIOAction.successful(true)
          }
          ...
          _ <- userMapSettingsDAO.createUserMapSettingsAction(
            userId = u.id.get, settings = mapSettings
          )
        } yield (uli, u)
      )
  }

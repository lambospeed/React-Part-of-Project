 ...
 */
class UserServiceImpl(protected val userServiceDAO: UserServiceDAO)(
  implicit val executionContext: ExecutionContext
) extends UserService {
  ...
  private def _getDefaultMapSettings(): MapSettings = {
    lazy val mapConfig: Config = ConfigFactory.load("map")
    lazy val locationConfig: Config =
      mapConfig.getConfig("map.settings.default.centerLocation")
    MapSettings(
      centerLocation = GeoLocation(
        latitude = locationConfig.getDouble("latitude"),
        longitude = locationConfig.getDouble("longitude")
      )
    )
  }

  def insert(identity: IdentityImpl): Future[UserXLoginInfo] = {
    userServiceDAO.userXLoginInfoDAO.insertIdentity(
      identity = identity, mapSettings = _getDefaultMapSettings()
    ).map(_._1)
  }

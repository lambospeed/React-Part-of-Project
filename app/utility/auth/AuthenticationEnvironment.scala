...
import aurita.daos.auth.{
  OAuth1InfoDAOInterface, OAuth2InfoDAOInterface, PasswordInfoDAOInterface
}
import aurita.utility.auth.providers.{
  TwentyThreeAndMeProfileParser, TwentyThreeAndMeProvider
}
import com.mohiva.play.silhouette.impl.providers.oauth2.InstagramProvider
import aurita.utility.auth.providers.{
  AmazonProvider, TwentyThreeAndMeProfileParser, TwentyThreeAndMeProvider
}

trait AuthenticationEnvironment extends PasswordInfoDAOInterface
  ...
  lazy val oAuth1TokenSecretProvider = new CookieSecretProvider(
    configuration.underlying.as[CookieSecretSettings](
  ...
  lazy val twentyThreeAndMeProvider = new TwentyThreeAndMeProvider(
    httpLayer = httpLayer,
    stateHandler = oAuth2StateHandler,
    settings = configuration.underlying.as[OAuth2Settings]("silhouette.twentythreeandme")
  )

  lazy val amazonProvider = new AmazonProvider(
    httpLayer = httpLayer,
    stateHandler = oAuth2StateHandler,
    settings = configuration.underlying.as[OAuth2Settings]("silhouette.amazon")
  )

  lazy val instagramProvider = new InstagramProvider(
    httpLayer = httpLayer,
    stateHandler = oAuth2StateHandler,
    settings = configuration.underlying.as[OAuth2Settings]("silhouette.instagram")
  )

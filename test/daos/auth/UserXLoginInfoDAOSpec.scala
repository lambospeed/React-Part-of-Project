...
import aurita.test.utility.TestEnvironment
...
trait UserXLoginInfoDAOSpecHelper extends TestEnvironment {
  ...
  ) extends TestSimpleApplication with TestData
}
  ...
  "The insert of an identity" should {
    "return the added user login info" in new WithDepsApplication {
      ...
      lazy val outUser: User = User(
        id = Some(2),
        groupId = 2,
        roleId = 3,  // Since there are already 2 rows from 13.sql
        statusId = 3,  // Since there are already 2 rows from 13.sql
      ...
      lazy val mapSettings: MapSettings = MapSettings(
        centerLocation = GeoLocation(latitude = 22.34, longitude = -79.110)
      )
      Await.result(
        userXLoginInfoDAO.insertIdentity(
          identity = inIdentity, mapSettings = mapSettings
        ),
        Duration.Inf
